...
int main()
{
    ...
    if (trans_uniform == -1)
        std::cout << "Variable 'trans' not found." << std::endl;

    GLint colour_uniform = glGetUniformLocation(shaders, "colour");
    if (colour_uniform == -1)
        std::cout << "Variable 'colour' not found." << std::endl;

    glUniform1f(trans_uniform, 0.5);
    glUniform4f(colour_uniform, 1.0, 1.0, 0.0, 1.0);
